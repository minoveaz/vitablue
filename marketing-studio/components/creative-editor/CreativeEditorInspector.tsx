...
export const CreativeEditorInspector: React.FC<CreativeEditorInspectorProps> = ({
  ...
  if (!activeScene) {
    return (
      <ModuleContextPanel label="Inspector" width="standard" onClose={onClose}>
    ...
    >
      ...
      {selectedLayer ? (
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-slate-200 pb-3">
            <div className="flex items-center gap-2">
              <span className="flex size-6 items-center justify-center rounded-md bg-primary/10 text-primary text-xs font-bold uppercase">
                {selectedLayer.type[0]}
              </span>
              <strong className="text-xs text-slate-800 capitalize">{selectedLayer.type} Layer</strong>
            </div>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                {selectedLayer.visible === false ? <EyeOff className="size-4 text-amber-500" /> : <Eye className="size-4" />}
              </button>
              ...
              >
                {selectedLayer.locked ? <Lock className="size-4 text-primary" /> : <Unlock className="size-4" />}
              </button>
          ...
          {selectedLayer.type === 'text' && (
            <div className="space-y-3">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Texto</label>
                <textarea
                  value={(selectedLayer as TextLayer).text}
                  onChange={(e) => onUpdateLayer(activeScene.id, selectedLayer.id, { text: e.target.value })}
                  rows={3}
                  className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs text-slate-800 focus:border-primary focus:outline-none"
                />
              </div>
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Color del Texto</label>
                <div className="flex items-center gap-2">
                  {['#ffffff', '#EE9B00', '#94D2BD', '#005F73'].map((c) => (
                    <button
                      ...
                      className="size-6 rounded-full border border-slate-300 shadow-xs focus:ring-2 focus:ring-primary"
                    />
                  ))}
                </div>
              </div>
          ...
          {selectedLayer.type === 'subtitle' && (
            <div className="space-y-3">
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Texto del Subtítulo</label>
                <textarea
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Estilo de Subtítulo</label>
                <select
          ...
          {selectedLayer.type === 'component' && (
            <div className="space-y-3">
              <span className="text-[10px] font-bold uppercase tracking-wider text-primary block">Props del Componente</span>
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Nombre Asesor</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Cargo / Especialidad</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Texto Botón WhatsApp</label>
                <input
                  type="text"
        ...
        <div className="space-y-5">
          ...
          {warnings.length > 0 && (
            <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900 space-y-1.5">
              <div className="flex items-center gap-1.5 font-bold text-amber-800">
                <AlertTriangle className="size-4 shrink-0" />
                <span>Advertencias de escena:</span>
              </div>
              {warnings.map((w, idx) => (
                <p key={idx} className="text-[11px] leading-relaxed text-amber-700">• {w}</p>
              ))}
            </div>
          )}

          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Plantilla de Escena</label>
            <select
              value={activeScene.templateId}
              onChange={(e) => onUpdateScene(activeScene.id, { templateId: e.target.value as SceneTemplateId })}
              className="w-full rounded-xl border border-slate-200 bg-white p-2.5 text-xs font-bold text-slate-800 focus:border-primary focus:outline-none"
            >
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Duración de Escena</label>
              <span className="font-mono text-xs font-bold text-primary">{(activeScene.durationInFrames / 30).toFixed(1)}s ({activeScene.durationInFrames} frames)</span>
            </div>
            <input
              type="range"
              min={30}
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Transición de Salida</label>
            <select
          ...
          <div className="pt-3 border-t border-slate-200 space-y-3">
            <span className="text-[10px] font-bold uppercase tracking-wider text-primary block">Contenido de la Escena</span>

            {activeScene.templateId === 'text_hook' && (
              <>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Titular Principal</label>
                  <textarea
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Texto de Badge</label>
                  <input
                    type="text"
            ...
            {activeScene.templateId === 'requirements_list' && (
              <>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Título de la Lista</label>
                  <input
                    type="text"
                    ...
                    className="w-full rounded-xl border border-slate-200 bg-white p-2 text-xs text-slate-800 mb-2"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Requisitos (uno por línea)</label>
                  <textarea
            ...
            {activeScene.templateId === 'advisor_cta' && (
              <>
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Llamada a la Acción (Botón)</label>
                  <input
                    type="text"
