    ...
    <div className="space-y-3">
      ...
      {scope === 'user' ? <div className="rounded-xl border border-dashed border-slate-700 p-6 text-center text-[10px] text-slate-500">Guarda una composición para verla en Míos.</div> : (
        <div className="grid grid-cols-2 gap-2.5">
          {templates.map((item) => {
            const template = MARKETING_TEMPLATE_PROJECT_BY_ID.get(item.projectId);
            // A fresh project prevents edits in the canvas from mutating the catalog composition.
            const preparedTemplate = template
              ? JSON.parse(JSON.stringify({ ...template, title: item.name })) as ImageProject
              : undefined;
            return <button key={item.id} type="button" disabled={!preparedTemplate} onClick={() => preparedTemplate && onLoadTemplate(preparedTemplate)} className="rounded-2xl border border-slate-800 bg-slate-950 p-2.5 text-left hover:border-primary transition-all">
              <div
                className="mb-2 flex w-full items-center justify-center rounded-xl border border-slate-700/40 bg-cover bg-center"
                style={{
                  aspectRatio: `${preparedTemplate?.preset.width ?? 1} / ${preparedTemplate?.preset.height ?? 1}`,
                  backgroundColor: preparedTemplate?.background.color,
                  backgroundImage: preparedTemplate?.background.gradient,
                }}
                aria-label={`Vista previa ${item.name}, formato ${preparedTemplate?.preset.aspectRatio ?? item.aspectRatio}`}
              >
                <span className="rounded bg-black/60 px-1.5 py-0.5 text-[9px] font-bold text-white">{preparedTemplate?.preset.aspectRatio ?? item.aspectRatio}</span>
              </div>
