  ...
  const content = (
    <div className="space-y-4 text-left">
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 sm:p-5 shadow-xs space-y-3">
        ...
        <div className="space-y-1" role="radiogroup" aria-label={isEnglish ? 'Filter by profile' : 'Filtrar por perfil'}>
          {profileOptions.map((opt) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-2.5 py-2 rounded-xl transition-all cursor-pointer text-left border ${
                  isSelected
              ...
              >
                ...
                <span
                  className={`text-[10px] font-black px-1.5 py-0.5 rounded-full shrink-0 ml-2 ${
                    isSelected
      ...
      <div className="bg-gradient-to-br from-primary-dark via-primary to-primary-dark text-white rounded-2xl p-4 sm:p-5 shadow-md border border-primary/30 space-y-3 relative overflow-hidden">
        <div className="absolute -top-12 -right-12 w-32 h-32 bg-accent/20 rounded-full blur-2xl pointer-events-none" />
        <div className="relative z-10 space-y-2.5">
          <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-accent text-[9px] font-black uppercase tracking-wider">
          ...
          <Link to="/validador-visado/" className="block pt-1">
            <Button
              variant="accent"
              size="sm"
              className="w-full font-black text-xs shadow-sm shadow-accent/20 py-2"
      ...
      <div className="bg-emerald-50/80 border border-emerald-200/80 rounded-2xl p-4 flex flex-col xl:flex-row items-start xl:items-center justify-between gap-3">
        <div className="space-y-0.5">
          <p className="text-xs font-bold text-emerald-950">
            {isEnglish ? 'Personalized Advice' : '¿Dudas con tu visado?'}
          </p>
          <span className="text-[11px] text-emerald-800 font-medium block">
            {isEnglish ? 'Instant response on WhatsApp' : 'Respuesta inmediata por WhatsApp'}
          </span>
        </div>
        <a
          href={whatsappUrl}
          ...
          className="w-full xl:w-auto px-3 py-2 bg-whatsapp-dark hover:bg-whatsapp text-white font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 shrink-0 transition-colors shadow-xs"
