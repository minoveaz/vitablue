...
const TrustCardGrid: React.FC<TrustCardGridProps> = ({ items }) => (
  <div className="grid gap-4 md:grid-cols-3">
    {items.map(({ title, description, illustration: Illustration, tone }) => (
      <article
        key={title}
        className={`group flex flex-col rounded-3xl border p-4 text-left shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg sm:p-5 lg:h-[190px] ${toneClasses[tone]}`}
      >
        <div className="mb-2 flex h-12 w-auto aspect-[4/3] items-center justify-start text-primary" aria-hidden="true">
          <Illustration />
        </div>
        <h3 className="text-base sm:text-lg font-display font-black leading-tight text-text-main">{title}</h3>
