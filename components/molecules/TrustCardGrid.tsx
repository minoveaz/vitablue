...
const TrustCardGrid: React.FC<TrustCardGridProps> = ({ items }) => (
  <div className="grid gap-4 md:grid-cols-3">
    {items.map(({ title, description, illustration: Illustration, tone }) => (
      <article
        key={title}
        className={`group flex flex-col rounded-3xl border p-5 text-left shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg sm:p-6 ${toneClasses[tone]}`}
      >
        ...
        <p className="mt-3 text-body-reg leading-relaxed text-text-secondary">{description}</p>
      </article>
    ))}
  </div>
);
